...
The workflow SHALL allow a user-selected Claude model and supported effort without changing persistent configuration. With no explicit per-review model override, it SHALL request Claude Opus 5 using claude-opus-5 regardless of ambient model defaults, and report the observed model when available; unknown effective settings SHALL remain unknown. The launcher SHALL enforce a finite timeout and turn bound, use no automatic model fallback, and report denied tools, usage exhaustion, unsupported settings, and interruption as incomplete or failed execution.
...
- **WHEN** a Claude cross-review has no explicit per-review model override, including when ambient settings select another model
...
- **THEN** the parent requests medium effort and records the scope-based reason
...
- **THEN** Claude is skipped by default, or receives low effort when the user explicitly requests that mechanical review
...
- **THEN** the parent requests high effort regardless of the number of changed lines, unless the user explicitly overrides it
...
- **THEN** the parent selects and logs effort from the remaining scope and risk, retaining high for consequential unresolved concerns and allowing low for a purely mechanical remainder
